...
class BurpExtender
  ...
  def registerExtenderCallbacks(callbacks)
    ...
    for arg in args
      ...
      elsif (arg.contains('-p=') || arg.contains('--port='))
        mongo_port = Integer.valueOf(arg.substring(arg.indexOf('=') + 1))
      elsif (arg.contains('-o=') || arg.contains('--ouput='))
        output_dir = arg.substring(arg.indexOf('=') + 1)
      elsif (arg.contains('-r=') || arg.contains('--report='))
        report_name = arg.substring(arg.indexOf('=') + 1)
      elsif (arg.contains('-t=') || arg.contains('--timeout='))
